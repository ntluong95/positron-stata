...
def detect_and_export_graphs_worker(stata, stlib, graphs_dir: str) -> list:
    ...
    """
    logging.debug(
        f"detect_and_export_graphs_worker: Platform={platform.system()}, graphs_dir={graphs_dir}"
    )

    if stata is None or stlib is None:
        logging.debug(
            "detect_and_export_graphs_worker: stata or stlib is None, returning empty list"
        )
        return []
    ...
    try:
        ...
        logging.info(
            f"detect_and_export_graphs_worker: Found {len(graph_names)} graph(s): {graph_names}"
        )
            ...
            try:
                ...
                if os.path.exists(graph_file):
                    ...
                    if file_size > 0:
                        ...
                        logging.info(
                            f"Successfully exported graph '{gname}' ({file_size} bytes) to {normalized_path}"
                        )
                    else:
...
"""
    ...
    def execute_stata_file(
        file_path: str, timeout: float = 600.0, log_file: str = None, working_dir: str = None
    ) -> tuple:
        """
                ...
                if stop_event is not None and stop_event.is_set():
                    ...
                    if worker_state == WorkerState.BUSY:
                        if handle_stop():
                            ...
                            send_result(
                                "_stop", "stop_skipped", "Stop already sent or not executing"
                            )
    ...
    try:
        ...
        if init_result is True:
            ...
            send_result(command_id="_init", status="init_failed", error=error_msg)
            ...
            try:
                ...
                if cmd_type == CommandType.EXIT:
                    ...
                    try:
                        if stata is None:
                            send_result(
                                command_id=cmd_id, status="error", error="Stata is not initialized"
                            )
                        else:
                            import sfi
                            import numpy as np
                            ...
                            if total_obs == 0:
                                ...
                                try:
                                    ...
                                    try:
                                        ...
                                        stata.run(
                                            "quietly gen long _stata_mcp_orig_obs = _n - 1",
                                            inline=False,
                                            echo=False,
                                        )
                                        ...
                                        stata.run(
                                            f"quietly keep if {if_condition}",
                                            inline=False,
                                            echo=False,
                                        )
                                ...
                                for column_name in column_names:
                                    try:
                                        column_labels[column_name] = (
                                            sfi.Data.getVarLabel(column_name) or ""
                                        )
                                    except Exception:
                                        column_labels[column_name] = ""
                    ...
                    send_result(
                        command_id=cmd_id, status="error", error=f"Unknown command type: {cmd_type}"
                    )
        ...
        for app_name in app_names:
            if app_name.endswith(".app"):
                ...
                exe_path = os.path.join(
                    stata_path, app_name, "Contents", "MacOS", f"stata-{edition_lower}"
                )
...
if __name__ == "__main__":
    ...
    p = multiprocessing.Process(
        target=worker_process, args=("test_worker", cmd_q, result_q, stata_path, "mp")
    )
    p.start()
