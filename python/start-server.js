...
const serverScript = path.join(extensionDir, "python", "stata_mcp_server.py");
...
async function startServer() {
  ...
  try {
    ...
    if (options.forcePort && (await isPortInUse(port))) {
      ...
      if (process.platform === "win32") {
        try {
          ...
          console.log(
            `Could not kill process using Windows method: ${error.message}`,
          );
        }
      } else {
        try {
          ...
          console.log(
            `Could not kill process using Unix method: ${error.message}`,
          );
        }
      }
      ...
      if (await isPortInUse(port)) {
        console.warn(
          `Warning: Port ${port} is still in use after kill attempt.`,
        );
      } else {
    ...
    if (fs.existsSync(pythonPathFile)) {
      ...
      console.log(
        `Python path file not found, using venv Python: ${pythonPath}`,
      );
    } else {
      // Try system Python
      pythonPath = process.platform === "win32" ? "py" : "python3";
      console.log(
        `No Python environment found, using system Python: ${pythonPath}`,
      );
    }
      ...
      if (process.platform === "win32") {
        ...
        if (serverProcess.stderr) {
          serverProcess.stderr.on("data", (data) => {
            ...
            if (
              message.includes("Picked up _JAVA_OPTIONS") ||
