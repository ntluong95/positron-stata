...
function promptUvInstallation() {
  ...
  console.log("==================================================");
  console.log(
    "Please install UV manually using one of the following commands:",
  );
...
async function setupPythonWithUv() {
  ...
  if (fs.existsSync(venvPath)) {
    ...
    try {
      await execPromise(
        `${process.platform === "win32" ? "rmdir /s /q" : "rm -rf"} "${venvPath}"`,
      );
      ...
      try {
        ...
        if (fs.existsSync(pythonPath)) {
          try {
            ...
            console.log(
              `Setup complete marker created at ${setupCompleteFile}`,
            );
  ...
  if (fs.existsSync(setupCompleteFile) && fs.existsSync(pythonPathFile)) {
    ...
    if (hoursSinceSetup < 24) {
      ...
      if (fs.existsSync(pythonPath) && isExecutable(pythonPath)) {
        console.log(
          `Python setup already complete (${hoursSinceSetup.toFixed(2)} hours ago)`,
        );
