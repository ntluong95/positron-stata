...
export interface StataConfiguration {
  ...
  startupTimeCommandsEnabled: boolean;
...
export function getStataConfiguration(): StataConfiguration {
  ...
  return {
    installationPath: normalizePath(
      config.get<string>("installationPath", "") || "",
    ),
    edition: edition === "se" || edition === "be" ? edition : "mp",
    serverHost:
      (config.get<string>("server.host", "localhost") || "localhost").trim() ||
      "localhost",
    serverPort: asPositiveInteger(
      config.get<number>("server.port", 4000),
      4000,
    ),
    ...
    runSelectionTimeout: asPositiveInteger(
      config.get<number>("runSelectionTimeout", 600),
      600,
    ),
    runFileTimeout: asPositiveInteger(
      config.get<number>("runFileTimeout", 600),
      600,
    ),
    workingDirectoryMode:
      config.get<WorkingDirectoryMode>("workingDirectory.mode", "dofile") ||
      "dofile",
    customWorkingDirectory: normalizePath(
      config.get<string>("workingDirectory.customPath", "") || "",
    ),
    resultDisplayMode:
      (config.get<ResultDisplayMode>("mcp.resultDisplayMode", "compact") ||
        "compact") === "full"
        ? "full"
        : "compact",
    maxOutputTokens: Math.max(
      0,
      asPositiveInteger(
        config.get<number>("mcp.maxOutputTokens", 10000),
        10000,
      ),
    ),
    multiSession: config.get<boolean>("multiSession.enabled", true) !== false,
    maxSessions: asPositiveInteger(
      config.get<number>("multiSession.maxSessions", 100),
      100,
    ),
    sessionTimeout: asPositiveInteger(
      config.get<number>("multiSession.sessionTimeout", 3600),
      3600,
    ),
    dataViewerMaxRows: Math.max(
      100,
      asPositiveInteger(
        config.get<number>("dataViewer.maxRows", 100000),
        100000,
      ),
    ),
    autocompleteRefreshAfterRun:
      config.get<boolean>("autocomplete.refreshAfterRun", true) !== false,
    autocompleteVariableRefreshEnabled:
      config.get<boolean>("autocomplete.variableRefresh.enabled", false) ===
      true,
    autocompleteVariableRefreshIntervalSeconds: Math.max(
      5,
      asPositiveInteger(
        config.get<number>("autocomplete.variableRefresh.intervalSeconds", 30),
        30,
      ),
    ),
    startupTimeCommandsEnabled:
      config.get<boolean>("sessionStartup.timeCommands.enabled", false) ===
      true,
    outlineNumberingShow:
      config.get<boolean>("outline.numberingShow", false) === true,
  };
}
