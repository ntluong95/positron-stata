...
function spawnNodeProcess(
	...
	return new Promise((resolve, reject) => {
		...
		child.on('exit', code => {
			...
			reject(new Error(`Python environment setup failed with exit code ${code}`));
		});
	});
}
...
export class StataServerManager implements vscode.Disposable {
	...
	async ensureStarted(installation?: StataInstallation): Promise<StataServerClient> {
	...
	private async startServer(installation?: StataInstallation): Promise<StataServerClient> {
		const configuration = getStataConfiguration();
		const resolvedInstallation = installation || await getPreferredStataInstallation();
		const installationPath = configuration.installationPath || resolvedInstallation?.installationPath;
		const edition = resolvedInstallation?.edition || configuration.edition;

		if (!installationPath) {
			throw new Error('No Stata installation was discovered. Set positron.stata.installationPath first.');
		}

		if (configuration.forcePort) {
			await this.killProcessOnPort(configuration.serverPort).catch(error => {
				this._logger.warn(`Unable to free port ${configuration.serverPort}: ${error}`);
			});
		}
		...
		const serverScript = path.join(this._context.extensionPath, 'python', 'stata_mcp_server.py');
		...
		const args = [
			...
			'--stata-path', installationPath,
			'--stata-edition', edition,
			'--log-level', configuration.debug ? 'DEBUG' : 'INFO',
			'--result-display-mode', configuration.resultDisplayMode,
			'--max-output-tokens', String(configuration.maxOutputTokens),
			'--max-sessions', String(configuration.maxSessions),
			'--session-timeout', String(configuration.sessionTimeout),
		];
		...
		this._logger.info(`Starting Stata MCP server on ${configuration.serverHost}:${configuration.serverPort}`);
		...
		this._serverProcess.on('exit', (code, signal) => {
			this._logger.info(`Stata MCP server exited with code ${code} and signal ${signal}`);
			this._serverProcess = undefined;
		});
	...
	private createClient(): StataServerClient {
		const configuration = getStataConfiguration();
		return new StataServerClient(configuration.serverHost, configuration.serverPort);
	}
	...
	private async waitForHealthyServer(client: StataServerClient): Promise<void> {
		...
		throw new Error(`Stata MCP server did not become healthy within ${HEALTH_TIMEOUT_MS}ms`);
	}
	...
	private async ensurePythonEnvironment(): Promise<void> {
		...
		const setupScript = path.join(this._context.extensionPath, 'python', 'check-python.js');
		if (!fs.existsSync(setupScript)) {
			throw new Error(`Python setup script not found: ${setupScript}`);
		}

		this._setupPromise = spawnNodeProcess(setupScript, this._context.extensionPath, this._logger)
			.finally(() => {
	...
	private hasUsablePythonEnvironment(): boolean {
		const setupMarker = path.join(this._context.extensionPath, '.setup-complete');
	...
	private readStoredPythonPath(): string | undefined {
		const pythonPathFile = path.join(this._context.extensionPath, '.python-path');
	...
	private async killProcessOnPort(port: number): Promise<void> {
		if (process.platform === 'win32') {
			await execShell(`FOR /F "tokens=5" %P IN ('netstat -ano ^| findstr :${port} ^| findstr LISTENING') DO taskkill /F /PID %P`);
			return;
		}
